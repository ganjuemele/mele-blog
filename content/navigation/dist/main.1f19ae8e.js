...
var simplifyURL = function simplifyURL(url) {
  return url.replace('https://', '').replace('http://', '').replace('www.', '').replace(/\/.*/, '').replace(/\.w*$/, '');
};
...
var render = function render(newChild, refChild) {
  hashMap.forEach(function (node, index) {
    var $nav = $("<div class=\"nav jump\">\n\t\t\t<div class=\"symbol\">".concat(node.logo, "</div>\n\t\t\t<div style=\"display: none\">").concat(simplifyURL(node.url), "</div>\n\t\t\t<div class=\"name\">").concat(node.urlName || simplifyURL(node.url), "</div>\n\t\t\t<div class=\"delNav\">\n\t\t\t\t<svg class=\"icon\">\n\t\t\t\t\t<use xlink:href=\"#icon-delete\"></use>\n\t\t\t\t</svg>\n\t\t\t</div>\n\t\t</div>")).insertBefore($add, refChild);
...
$('.jump').on('click', function (e) {
  var currentURL = e.currentTarget.children[1].innerText;
  window.open('http://' + currentURL);
});
$add.on('click', function () {
  var url = window.prompt('输入您要添加的站点：');
  var urlName = window.prompt('输入站点名称：');
  hashMap.push({
    urlName: urlName,
